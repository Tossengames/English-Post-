...
def create_vertical_video(hook, problem, solution, output_path):
    """Create vertical video with problem-solution format - EDITED: CTA moved down, template labels removed"""
    temp_dir = tempfile.mkdtemp()
    ...
    # Create a filter script file - EDITED: Removed template labels, moved CTA down
    filter_script = os.path.join(temp_dir, "filter.txt")
    with open(filter_script, 'w') as f:
        f.write(f"""
        [0:v]{video_filters},
        drawtext=text='{wrapped_hook}':fontcolor={hook_color}:fontsize=65:box=1:boxcolor=white@1.0:boxborderw=10:x=(w-text_w)/2:y=250,
        drawtext=text='{wrapped_problem}':fontcolor={problem_color}:fontsize=45:box=1:boxcolor=white@1.0:boxborderw=5:x=(w-text_w)/2:y=450,
        drawtext=text='{wrapped_solution}':fontcolor={solution_color}:fontsize=45:box=1:boxcolor=white@1.0:boxborderw=5:x=(w-text_w)/2:y=700,
        drawtext=text='Follow for more mom solutions!':fontcolor=darkmagenta:fontsize=40:x=(w-text_w)/2:y=1600
        """)
